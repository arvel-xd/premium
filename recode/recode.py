...
def banner():
	clear()
	wel = '# SELAMAT DATANG AUTHOR ANSYAH ARVEL JANGAN LUPA COLI'
	wel2 = mark(wel, style='green')
	sol().print(wel2)
	au='   ••• Recode By: ARVEL••• Github :Gk Punya ••• \n  _____   ______ _______ _______ _____ _     _ _______ \n |_____] |_____/ |______ |  |  |   |   |     | |  |  |   \n |       |    \_ |______ |  |  | __|__ |_____| |  |  | PREMIUM KATA ARVEL'                                             
...
def menu(my_name,my_id):
	...
	gh = 'github.com/ARVEL'
	cetak(nel('\tSelamat Datang  [yellow]%s[white] Sayang Arvel'%(my_name)))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r [ARVEL] {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x} ➞ {h}Arvel jangan lupa sama Tuhan ya\n\t{x} ➞ {h}Arvel kamu ganteng baik lagi \n\t{x} ➞ {h}Arvel sayang kalian semua \n\t{x} ➞ {h}Arvel sebelum crack doa dulu\n\t{x} ➞ {h}Arvel kamu hanya manusia biasa{x}')
	time.sleep(3)
	login()
